...
def load_data():
    ...
    "mssql+pyodbc://admin:Streamlit123.@ecommerce-db.c9ycgk8s81st.eu-north-1.rds.amazonaws.com,1433/E-commerce?driver=ODBC+Driver+17+for+SQL+Server"
)
...
def project_intro():
    ...
    # Functional button
    if st.button("👉 Continue to Business Context"):
...
# === USER BEHAVIOR PAGE ===
    if selected_page == "User Behavior":
        ...
        df_items_filtered = df_order_items.copy()
        df_refund_filtered = df_order_item_refunds.copy()
...
# TOTAL METRICS
        ...
        Net_Revenue = df_items_filtered['price_usd'].sum() - df_order_item_refunds["refund_amount_usd"].sum()
        total_cost = df_items_filtered['cogs_usd'].sum()
        profit = Net_Revenue - total_cost - df_order_item_refunds['refund_amount_usd'].sum()
...
# Section 1: Core Metrics
        ...
        with row1_col2:
            custom_kpi("🌐 Total Sessions", f"{total_sessions/1_000_000:,.2f}M")
        with row1_col3:
        ...
        with row2_col3:
            custom_kpi("📉 Net Cost (COGS)", f"${total_cost / 1_000_000:,.2f}M")
...
# Section 2: Buyer Insights
        ...
        with row6_col3:
            custom_kpi("⏱️ Avg Days 1st → 2nd Purchase", f"{avg_gap:.2f} days")
...
# Section 3: User Behavior
        ...
        with row7_col1:
            custom_kpi("🧍 One-time Users", f"{one_time_users/1_000_000:,.2f}M")
        with row7_col2:
            custom_kpi("👥 Returning Users", f"{returning_users/1_000_000:,.2f}M")
        with row7_col3:
...
# Section 4: Refunds
        ...
        with row9_col1:
            custom_kpi("💸 Total Refund Amount", f"${Total_Refund / 1_000_000:,.2f}M")
        with row9_col2:
...
# Plot
        ...
        sns.set(style="white")
        sns.barplot(data=orders_by_channel,x='utm_source',y='total_orders',palette='pastel',
                 ax=ax1 )
...
# Aggregate revenue
        revenue_by_channel = (orders_with_channel.groupby('utm_source')['price_usd']
                        .sum().reset_index()
                        .sort_values(by='price_usd', ascending=False) )
